...
from jason_pastoleiro import CURRENT_DAY, CURRENT_HOUR, CURRENT_MONTH, Jason, Kid, Receiver, Sector, turn_in_dataclass, CURRENT_MONTH_DAY_LIST
...
kid_3 = Kid('Froid', 7, date(2022, 8, 14), date(2022, CURRENT_MONTH, CURRENT_DAY), 114, 'Bob', '')
kid_mock = kid.make_dict()
kid_mock_2 = kid_2.make_dict()
kid_mock_3 = kid_3.make_dict()
...
class TestJason(TestCase):
    ...
    def test_delete_by_checkout_day(self):
        print('Testando delete by_checkout day')
        self.jason.my_receiver.add_kid_to_sector(kid_mock_3)
        self.jason._store_in_data()
        self.assertIn(kid_mock_3, self.jason.my_receiver.kids_sector_list['kids club'])
        self.jason.delete_by_checkout_day('kids club', CURRENT_HOUR)
        self.assertNotIn(kid_mock_3, self.jason.my_receiver.kids_sector_list['kids club'])
